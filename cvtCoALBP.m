function F = cvtCoALBP(img, s, r, config)
%
...
%CoALBP use 4 neighbor pixels, the LBP index is a value between 0 to 15. 16 comes from here and it is used for encoding for LBP pairs.
X=double(X>0);
...
F(:,1) = hist(Y1(:), 1:(16*16));%creates a histogram bar chart of the elements in vector x
F(:,2) = hist(Y2(:), 1:(16*16));
F(:,3) = hist(Y3(:), 1:(16*16));
F(:,4) = hist(Y4(:), 1:(16*16));
F = F(:);
